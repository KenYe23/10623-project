...
class PaperVizProcessor:
    ...
    async def _run_critic_iterations(
        ...
        for round_idx in range(max_rounds):
            ...
            if critic_suggestions == "No changes needed.":
                print(
                    f"[Critic Round {round_idx}] No changes needed. Stopping iteration."
                )
                break
            ...
            if new_image_key in data and data[new_image_key]:
                ...
                print(
                    f"[Critic Round {round_idx}] Completed iteration. Visualization SUCCESS."
                )
            else:
                print(
    ...
    async def _run_parallel_debate_iterations(
        self,
        data: Dict[str, Any],
        task_name: str,
        max_rounds: int = 3,
        ...
        for round_idx in range(max_rounds):
            ...
            if critic_suggestions == "No changes needed.":
                print(
                    f"[Parallel Debate Round {round_idx}] No changes needed. Stopping."
                )
                break
            ...
            if new_image_key in data and data[new_image_key]:
                current_best_image_key = new_image_key
                print(
                    f"[Parallel Debate Round {round_idx}] Completed. Visualization SUCCESS."
                )
            else:
                print(
                    f"[Parallel Debate Round {round_idx}] Visualization FAILED. Rolling back to {current_best_image_key}"
                )
                break
    ...
    async def process_single_query(
        ...
        if exp_mode == "vanilla":
            ...
            if not already_retrieved:
                data = await self.retriever_agent.process(
                    data, retrieval_setting=retrieval_setting
                )
            ...
            data = await self._run_critic_iterations(
                data, task_name, max_rounds=max_rounds, source="planner"
            )
            if "demo" in exp_mode:
                do_eval = False
            ...
            max_rounds = data.get(
                "max_critic_rounds", self.exp_config.max_critic_rounds
            )
            data = await self._run_critic_iterations(
                data, task_name, max_rounds=max_rounds, source="planner"
            )
            ...
            data = await self._run_parallel_debate_iterations(
                data, task_name, max_rounds=max_rounds, source="planner"
            )
        ...
        if do_eval:
            data_with_eval = await self.evaluation_function(
                data, exp_config=self.exp_config
            )
    ...
    async def process_queries_batch(
        ...
        if needs_retrieval and data_list:
            is_per_sample = retrieval_setting == "auto" and (
                ...
                or (
                    self.retriever_agent.model_name.startswith("bedrock/")
                    and "anthropic" in self.retriever_agent.model_name
                )
            )
            if is_per_sample:
                ...
                print(
                    f"[Retriever] Running per-sample retrieval for {len(data_list)} samples (with prompt caching)..."
                )
                sem = asyncio.Semaphore(5)  # limit concurrency to avoid rate limits

                async def _retrieve_one(d):
                    async with sem:
                        return await self.retriever_agent.process(
                            dict(d), retrieval_setting=retrieval_setting
                        )
                ...
                for i, res in enumerate(retrieval_results):
                    if isinstance(res, Exception):
                        print(
                            f"[Retriever] Sample {i} failed: {res}. Using empty refs."
                        )
                        data_list[i]["top10_references"] = []
                        data_list[i]["retrieved_examples"] = []
                    else:
                        data_list[i]["top10_references"] = res.get(
                            "top10_references", []
                        )
                        data_list[i]["retrieved_examples"] = res.get(
                            "retrieved_examples", []
                        )

                ok_count = sum(
                    1 for r in retrieval_results if not isinstance(r, Exception)
                )
                ...
                first_data = await self.retriever_agent.process(
                    first_data, retrieval_setting=retrieval_setting
                )
                ...
                print(
                    f"[Retriever] Done. Retrieved {len(first_data.get('top10_references', []))} references."
                )
                ...
                for dim in eval_dims:
                    ...
                    if winner_key in result_data:
                        ...
                        if total > 0:
                            ...
                            t_cnt = (
                                winners.count("Tie")
                                + winners.count("Both are good")
                                + winners.count("Both are bad")
                            )
                            ...
                            postfix_dict[display_key] = (
                                f"{m_rate:.0f}/{t_rate:.0f}/{h_rate:.0f}"
                            )
    ...
    async def evaluation_function(
        ...
        data = await get_score_for_image_referenced(
            ...
            model_name=exp_config.main_model_name,
